...
class SwaggerSpec2 extends ScalatraSpec with JsonMatchers {
  ...
  val swagger = new Swagger(
    "2.0",
    "1.0.0",
    TestFixtures.apiInfo,
    extraSwaggerDefinition = Some(extraSwagger)
  )
  ...
  swagger.addAuthorization(
    ApiKey(
      "Authorization1",
      "query",
      "you must register your app to receive an apikey"
    )
  )
  swagger.addAuthorization(
    OAuth(
    ...
    paths flatMap { case (path, JObject(x)) =>
      x map { case (method, operation) =>
        ...
        (JString(operationId) must_== operation \ "operationId") and
          verifyOperation(
            operation,
            swaggerJsonJValue \ "paths" \ path \ method,
            operationId
          )
      }
        ...
        case "responses" =>
          ...
          val r = af map { case (af_code, af_value) =>
            val mm = verifyFields(
              af_value,
              ef collectFirst {
                case (ef_code, ef_value) if ef_code == af_code => ef_value
              } getOrElse JNothing,
              "schema",
              "description"
            )
...
class SwaggerTestServlet(protected val swagger: Swagger)
  ...
  implicit val StringFormat: JsonFormat[String] = JsonFormat.GenericFormat(
...
class PetData {
  ...
  def findPetsByTags(tag: String): Map[String, Pet] = {
    ...
    pets.collect {
      case pet if (tags & pet.tags.map(_.name).toSet).nonEmpty =>
        pet.name -> pet
    }.toMap
  }
