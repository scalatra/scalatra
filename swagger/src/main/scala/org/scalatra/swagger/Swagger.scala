...
trait SwaggerEngine {
  ...
  def addAuthorization(auth: AuthorizationType): Unit =
...
object Swagger {
  ...
  private[swagger] def collectModels(
    ...
    if (excludes contains tpe.erasure) {
      ...
      (tpe.isCollection && tpe.typeArgs.nonEmpty) || (tpe.isOption && tpe.typeArgs.nonEmpty)
    ) {
    ...
    val mp = ModelProperty(
      `type` = DataType.fromScalaType(
        if (prop.returnType.isOption)
          ctorParam
            .map(_.argType.typeArgs.head)
            .getOrElse(prop.returnType.typeArgs.head)
  ...
  def modelToSwagger(klass: ScalaType): Option[Model] =
    if (
      Reflector.isPrimitive(klass.erasure) || Reflector.isExcluded(
        klass.erasure,
        excludes.toSeq
      )
    ) None
      ...
      val fields = klass.erasure.getDeclaredFields.toList collect {
        case f: Field if f.getAnnotation(classOf[ApiModelProperty]) != null =>
          ...
          val position =
            if (annotation.position() == Integer.MAX_VALUE) None
            else Some(annotation.position())
          val minimumValue =
            if (annotation.minimumValue().isNaN) None
            else Option(annotation.minimumValue())
          val maximumValue =
            if (annotation.maximumValue().isNaN) None
            else Option(annotation.maximumValue())
  ...
  def register(
    ...
    _docs += listingPath -> Api(
      ...
      (produces ::: endpoints.flatMap(
        _.operations.flatMap(_.produces)
      )).distinct,
      (consumes ::: endpoints.flatMap(
        _.operations.flatMap(_.consumes)
      )).distinct,
      (protocols ::: endpoints.flatMap(
        _.operations.flatMap(_.schemes)
      )).distinct,
      endpoints,
      s.models.toMap,
...
object DataType {
  ...
  private[this] val StringTypes =
    Set[Class[?]](classOf[String], classOf[java.lang.String])
  private[this] def isString(klass: Class[?]) = StringTypes contains klass
  private[this] val BoolTypes =
    Set[Class[?]](classOf[Boolean], classOf[java.lang.Boolean])
  ...
  private[swagger] def fromClass(klass: Class[?]): DataType = fromScalaType(
    Reflector.scalaTypeOf(klass)
  )
  private[swagger] def fromScalaType(st: ScalaType): DataType = {
    val klass =
      if (st.isOption && st.typeArgs.nonEmpty) st.typeArgs.head.erasure
      else st.erasure
    if (
      classOf[Unit]
        .isAssignableFrom(klass) || classOf[Void].isAssignableFrom(klass)
    ) this.Void
    else if (isString(klass)) this.String
    else if (
      classOf[Byte].isAssignableFrom(klass) || classOf[java.lang.Byte]
        .isAssignableFrom(klass)
    ) this.Byte
    else if (
      classOf[Long].isAssignableFrom(klass) || classOf[java.lang.Long]
        .isAssignableFrom(klass)
    ) this.Long
    else if (isInt(klass)) this.Int
    else if (
      classOf[Float].isAssignableFrom(klass) || classOf[java.lang.Float]
        .isAssignableFrom(klass)
    ) this.Float
      ...
      classOf[scala.collection.Set[?]]
        .isAssignableFrom(klass) || classOf[java.util.Set[?]]
        .isAssignableFrom(klass)
    ) {
      if (st.typeArgs.nonEmpty) GenSet(fromScalaType(st.typeArgs.head))
      else GenSet()
    } else if (
      classOf[collection.Seq[?]]
        .isAssignableFrom(klass) || classOf[java.util.List[?]]
        .isAssignableFrom(klass)
    ) {
  ...
  private[this] def isDate(klass: Class[?]) =
  ...
  private[this] def isDateTime(klass: Class[?]) =
...
case class Model(
  ...
  def getVisibleProperties: Seq[(String, ModelProperty)] =
    properties.filter(!_._2.hidden)
}
...
case class ApiKey(
    keyName: String,
    passAs: String = "header",
    description: String = ""
) extends AuthorizationType {
  override val `type` = "apiKey"
}

case class BasicAuth(keyName: String, description: String = "")
    extends AuthorizationType {
