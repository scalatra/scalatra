...
import org.scalatra.ServletCompat.{Filter, Servlet}
...
object SwaggerSupportSyntax {
  ...
  class SinatraSwaggerGenerator(matcher: SinatraRouteMatcher) {
    def toSwaggerPath: String =
      BuilderGeneratorParser(matcher.toString)(Builder("")).get
  ...
  trait SwaggerParameterBuilder {
    ...
    def position(position: Int): this.type = {
      _position = Some(position); this
    }
  ...
  class ModelParameterBuilder(val initialDataType: DataType)
      extends SwaggerParameterBuilder {
    dataType(initialDataType)
  }
  ...
  class OperationBuilder(val resultClass: DataType) {
    ...
    def responseMessages(errs: ResponseMessage*): this.type = {
    ...
    def schemes(values: String*): this.type = {
      _schemes :::= values.toList; this
    }
...
trait SwaggerSupportSyntax extends Initializable with CorsSupport {
  ...
  private[this] def throwAFit =
    throw new IllegalStateException(
      "I can't work out which servlet registration this is."
    )
      ...
      this match {
        case _: Filter =>
          ...
          val registration = registrations.find(
            _.getClassName == getClass.getName
          ) getOrElse throwAFit
  ...
  /** Registers a model for swagger, this method reflects over the class and
    * collects all non-primitive classes and adds those to the swagger defintion
    *
    * @tparam T
  ...
  implicit def parameterBuilder2parameter(
      pmb: SwaggerParameterBuilder
  ): Parameter = pmb.result

  private[this] def swaggerParam[T: Manifest](
      name: String,
      liftCollection: Boolean = false,
      allowsCollection: Boolean = true,
      allowsOption: Boolean = true
  ): ParameterBuilder[T] = {
    val st = Reflector.scalaTypeOf[T]
    if (st.isCollection && !allowsCollection)
      sys.error("Parameter [" + name + "] does not allow for a collection.")
    if (st.isOption && !allowsOption)
  ...
  protected def bodyParam[T: Manifest: NotNothing](
      name: String
  ): ParameterBuilder[T] = swaggerParam[T](name).fromBody
  ...
  protected def swaggerEndpointEntries(
      extract: (Route, HttpMethod) => Operation
  ) =
    for {
...
trait SwaggerSupport
  ...
  protected implicit def operationBuilder2operation[T](
      bldr: OperationBuilder
  ): Operation = bldr.result
  protected def apiOperation[T: Manifest: NotNothing](
      operationId: String
  ): OperationBuilder = {
    registerModel[T]()
  ...
  private def makeOperationBuilder(
      operationId: String,
      dataType: DataType
  ): OperationBuilder = {
  ...
  /** Returns a list of operations based on the given route. The default
    * implementation returns a list with only 1 operation.
    */
  protected def extractOperation(
    ...
    op map (_.copy(method = method)) getOrElse {
      val theParams = route.metadata.get(
        Symbols.Parameters
      ) map (_.asInstanceOf[List[Parameter]]) getOrElse Nil
      ...
      val description =
        route.metadata.get(Symbols.Description) map (_.asInstanceOf[String])
      val operationId = route.metadata.get(
        Symbols.OperationId
