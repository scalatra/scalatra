...
private[reflect] object ScalaSigReader {
    ...
    def read(current: Class[?]): Option[MethodSymbol] =
      if (current == null)
        ...
        findClass(current)
          ...
          .orElse(current.getInterfaces.to(LazyList).map(read).collectFirst {
            case Some(m) =>
              m
          })
    findArgTypeForField(
      read(clazz).getOrElse(
  ...
  def findScalaSig(clazz: Class[?]): Option[ScalaSig] =
    parseClassFileFromByteCode(clazz).orElse(
      Option(clazz.getDeclaringClass).flatMap(findScalaSig)
    )
  ...
  def companionClass(clazz: Class[?], classLoaders: Iterable[ClassLoader]) = {
    val path =
      if (clazz.getName.endsWith("$")) clazz.getName
      else "%s$".format(clazz.getName)
    val c = resolveClass(path, classLoaders)
    if (c.isDefined) c.get
    else sys.error("Could not resolve clazz='%s'".format(path))
  }
  ...
  def resolveClass[X <: AnyRef](
      c: String,
      classLoaders: Iterable[ClassLoader]
  ): Option[Class[X]] = classLoaders match {
    case Nil =>
      sys.error(
