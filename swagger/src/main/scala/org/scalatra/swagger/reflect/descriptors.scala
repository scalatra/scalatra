...
object ManifestScalaType {
  ...
  def apply[T](mf: Manifest[T]): ScalaType = {
    ...
    else if (mf.runtimeClass == classOf[Short] || mf.runtimeClass == classOf[java.lang.Short])
      ManifestScalaType.ShortType
    else if (mf.runtimeClass == classOf[Float] || mf.runtimeClass == classOf[java.lang.Float])
      ManifestScalaType.FloatType
    else if (mf.runtimeClass == classOf[Double] || mf.runtimeClass == classOf[java.lang.Double])
    ...
    else if (mf.runtimeClass == classOf[Boolean] || mf.runtimeClass == classOf[java.lang.Boolean])
      ManifestScalaType.BooleanType
    else if (mf.runtimeClass == classOf[String] || mf.runtimeClass == classOf[java.lang.String])
...
class ManifestScalaType(val manifest: Manifest[?]) extends ScalaType {
  ...
  val typeArgs: Seq[ScalaType] = manifest.typeArguments
    .map(ta => Reflector.scalaTypeOf(ta)) ++ (if (erasure.isArray) List(Reflector.scalaTypeOf(erasure.getComponentType))
                                              else Nil)
  ...
  lazy val simpleName: String =
    rawSimpleName + (if (typeArgs.nonEmpty) typeArgs.map(_.simpleName).mkString("[", ", ", "]")
  ...
  def copy(
    ...
    else if (erasure == classOf[BigDecimal] || erasure == classOf[java.math.BigDecimal])
      ManifestScalaType.BigDecimalType
...
case class ClassDescriptor(
  ...
  lazy val mostComprehensive: Seq[ConstructorParamDescriptor] =
    if (constructors.isEmpty) Seq.empty else constructors.maxBy(_.params.size).params
}
