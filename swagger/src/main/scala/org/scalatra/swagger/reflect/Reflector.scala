...
object Reflector {
  ...
  def scalaTypeOf(clazz: Class[?]): ScalaType = ManifestScalaType(
  ...
  def describe(
      st: ScalaType,
      paranamer: ParameterNameReader = ParanamerReader
  ): ObjectDescriptor =
      ...
      def properties: Seq[PropertyDescriptor] = {
        def fields(clazz: Class[?]): List[PropertyDescriptor] = {
          ...
          while (ls.hasNext) {
            ...
            if (
              !(Modifier.isStatic(mod) || Modifier.isTransient(mod) || Modifier
  ...
  def defaultValue(compClass: Class[?], compObj: AnyRef, argIndex: Int) =
    allCatch.withApply(_ => None) {
      Option(
        compClass.getMethod("%s$%d".format(ConstructorDefault, argIndex + 1))
      ) map { meth => () =>
        meth.invoke(compObj)
      }
    }
