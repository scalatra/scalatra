...
trait ValueType[T] {
  ...
  def verifying(validator: (T) => Seq[(String, String)]): ValueType[T] =
    new VerifyingValueType(
      this,
      (value: T, params: Map[String, Seq[String]]) => validator(value)
    )
...
/** The base class for the single field ValueTypes.
  */
abstract class SingleValueType[T](constraints: Constraint*)
    extends ValueType[T] {
  ...
  private def validaterec(
    ...
    constraints match {
      case (x :: rest) =>
        x.validate(name, value, params, messages) match {
...
/** ValueType wrapper to verify the converted value. An instance of this class
  * is returned from only [[ValueType#verifying]].
  *
  * @param valueType
  *   the wrapped ValueType
  * @param validator
  *   the function which verifies the converted value
  */
private[forms] class VerifyingValueType[T](
  ...
  def convert(
    ...
    valueType.convert(name, params, messages)
...
abstract class MappingValueType[T] extends ValueType[T] {
  ...
  def validate(
    ...
    fields.flatMap { case (fieldName, valueType) =>
      valueType.validate(
