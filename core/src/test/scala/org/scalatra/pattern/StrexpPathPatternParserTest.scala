 ...
 */
class StrexpPathPatternParserTest extends FunSuite with ShouldMatchers {
  test("unanchored segment") {
    val PathPattern(re, names) = StrexpPathPatternParser("/foo(/bar)")
    re.toString should equal ("""\A/foo(?:/bar)?\Z""")
    names should equal (Nil)
  }

  test("static string") {
    val PathPattern(re, names) = StrexpPathPatternParser("foo")
    re.toString should equal ("""\Afoo\Z""")
    names should equal (Nil)
  }

  test("dynamic segment") {
    val PathPattern(re, names) = StrexpPathPatternParser(":foo.example.com")
    re.toString should equal ("""\A([^#/.?]+)\.example\.com\Z""")
    names should equal (List("foo"))
  }

  test("dynamic segment with leading underscore") {
    val PathPattern(re, names) = StrexpPathPatternParser(":_foo.example.com")
    re.toString should equal ("""\A([^#/.?]+)\.example\.com\Z""")
    names should equal (List("_foo"))
  }

  test("skips invalid group names: 123") {
    val PathPattern(re, names) = StrexpPathPatternParser(":123.example.com")
    re.toString should equal ("""\A:123\.example\.com\Z""")
    names should equal (Nil)
  }

  test("skips invalid group names: $") {
    val PathPattern(re, names) = StrexpPathPatternParser(":$.example.com")
    re.toString should equal ("""\A:\$\.example\.com\Z""")
    names should equal (Nil)
  }
  ...
  test("dynamic segment inside optional segment") {
    val PathPattern(re, names) = StrexpPathPatternParser("foo(.:extension)")
    re.toString should equal ("""\Afoo(?:\.([^#/.?]+))?\Z""")
    names should equal (List("extension"))
  }

  test("glob segment") {
    val PathPattern(re, names) = StrexpPathPatternParser("src/*files")
    re.toString should equal  ("""\Asrc/(.+)\Z""")
    names should equal (List("files"))
  }

  test("glob segment at the beginning") {
    val PathPattern(re, names) = StrexpPathPatternParser("*files/foo.txt")
    re.toString should equal  ("""\A(.+)/foo\.txt\Z""")
    names should equal (List("files"))
  }

  test("glob segment in the middle") {
    val PathPattern(re, names) = StrexpPathPatternParser("src/*files/foo.txt")
    re.toString should equal  ("""\Asrc/(.+)/foo\.txt\Z""")
    names should equal (List("files"))
  }

  test("multiple glob segments") {
    val PathPattern(re, names) = StrexpPathPatternParser("src/*files/dir/*morefiles/foo.txt")
  ...
  test("consecutive optional segment") {
    val PathPattern(re, names) = StrexpPathPatternParser("/foo(/bar)(/baz)")
