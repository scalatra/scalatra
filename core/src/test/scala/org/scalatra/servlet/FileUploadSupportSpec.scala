...
class FileUploadSupportSpecServlet extends ScalatraServlet with FileUploadSupport {
  def headersToHeaders(implicit request: HttpServletRequest, response: HttpServletResponse) = {
    request.getHeaderNames.filter(_.startsWith("X")).foreach(header =>
      response.setHeader(header, request.getHeader(header))
    )
  }

  def fileParamsToHeaders(implicit request: HttpServletRequest, response: HttpServletResponse) {
  ...
  def paramsToHeaders(implicit request: HttpServletRequest, response: HttpServletResponse) {
