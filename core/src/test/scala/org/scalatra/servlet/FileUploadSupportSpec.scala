...
class FileUploadSupportSpec extends MutableScalatraSpec {
  ...
  def postExample[A](f: => A): A = {
    ...
    val headers =
      Map("X-Header" -> "I'm a header", "X-Header2" -> "I'm another header")
    ...
    "make all files available through fileParams" in {
      postExample {
        (header("File-text-Name") must_== "lorem_ipsum.txt") and
          ...
          (header(
            "File-text-SHA"
          ) must_== "b3572a890c5005aed6409cf81d13fd19f6d004f0") and
          (header("File-binary-Name") must_== "smiley.png") and
          (header("File-binary-Size") must_== "3432") and
          (header(
            "File-binary-SHA"
          ) must_== "0e777b71581c631d056ee810b4550c5dcd9eb856")
      }
    }
    ...
    "keep file params on pass" in {
      postPass {
        (header("File-text-Name") must_== "lorem_ipsum.txt") and
          ...
          (header(
            "File-text-SHA"
          ) must_== "b3572a890c5005aed6409cf81d13fd19f6d004f0")
      }
    }
  ...
  "GetAllFileItems" should {
    ...
    "return all file number" in {
      ...
      val filename =
        "core/src/test/resources/org/scalatra/servlet/lorem_ipsum.txt"
      val files =
        Map("doc1" -> new File(filename), "doc2" -> new File(filename))
