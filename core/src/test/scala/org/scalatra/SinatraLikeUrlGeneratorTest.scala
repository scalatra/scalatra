...
class SinatraLikeUrlGeneratorTest extends AnyFunSuite with Matchers {
  ...
  test(
    "String route with optional trailing slash reverses to url with no slash"
  ) {
    url("/foo/?") should equal("/foo")
  }
  ...
  test("Route with two named parameters replaces both parameters") {
    url(
      ...
      "bar" -> "moonless",
      "rebar" -> "midnight"
    ) should equal("/foo/moonless/and/midnight")
  }
  ...
  test("Named of path containing a dot get replaced anyway") {
    url(
      ...
      "file" -> "TroutLauncher",
      "ext" -> "scala"
    ) should equal("/dot-outside-named-param/TroutLauncher.scala")
  }
  ...
  test("Unexpected parameters are added as query string") {
    url(
      ...
      "bar" -> "pepper",
      "unexpected" -> "surprise"
    ) should equal("/foo/pepper?unexpected=surprise")
  }
  ...
  test("Many splat parameters get replaced") {
    url(
      ...
      "in",
      "japan"
    ) should equal("/mixing-multiple-splats/made/foo/in/japan")
  }

  test("Mix named and splat") {
    url(
      "/mix-named-and-splat-params/:foo/*",
      Map("foo" -> "deep"),
      Seq("purple")
    ) should equal("/mix-named-and-splat-params/deep/purple")
  }
