...
/** Defines type classes and helper methods for well known content-negotiation
  * headers.
  */
object ContentNegotiation {
  ...
  trait Format[T] extends RegexParsers {
    ...
    def connegs: Parser[List[Option[ContentNegotiation[T]]]] =
      repsep(conneg, ",")
  ...
  // - Value retrieval -------------------------------------------------------------------------------------------------
  // -------------------------------------------------------------------------------------------------------------------
  /** Retrieves all supported values of the specified content-negotiation
    * header.
    *
    * Note that any value declared in the header but not supported by the system
    * will be absent from the list. For example, an `Accept-Charset` value of
    * `utf-256` will yield an empty list.
    *
    * Additionally, this method swallows errors silently. An invalid header
    * value will yield an empty list rather than an exception.
    */
  def values[T](name: String)(implicit
      req: HttpServletRequest,
      format: Format[T]
  ): List[ContentNegotiation[T]] = {
    val header = req.getHeader(name)
    if (header == null) List()
    else format.values(header.trim())
  }

  /** Retrieves the preferred supported value for the specified
  ...
  def preferredEncoding(implicit
      req: HttpServletRequest
  ): Option[ContentEncoding] = preferredValue(AcceptEncoding)
  def acceptedEncodings(implicit
      req: HttpServletRequest
  ): List[ContentNegotiation[ContentEncoding]] = values(
    AcceptEncoding
  )
  ...
  def acceptedCharsets(implicit
      req: HttpServletRequest
  ): List[ContentNegotiation[Charset]] = values(AcceptCharset)
}
