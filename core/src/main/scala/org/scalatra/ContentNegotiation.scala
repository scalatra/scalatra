...
object ContentNegotiation {
  ...
  trait Format[T] extends RegexParsers {
    ...
    def parameter: Parser[(String, String)] = (token ~ (valueSep ~> (token | quotedString))) ^^ { case token ~ value =>
      (token, value)
    }
  ...
  def acceptedEncodings(implicit req: HttpServletRequest): List[ContentNegotiation[ContentEncoding]] = values(
    AcceptEncoding
  )
