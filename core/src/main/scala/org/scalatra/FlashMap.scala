...
/** A FlashMap is the data structure used by [[org.scalatra.FlashMapSupport]] to
  * allow passing temporary values between sequential actions.
  *
...
class FlashMap extends Serializable {
  ...
  /** Clears the flag for the specified key so its entry is not removed on the
    * next sweep.
    */
  ...
  /** Sets a value for the current request only. It will be removed before the
    * next request unless explicitly kept. Data put in this object is available
    * as usual:
    * {{{
...
/** Allows an action to set key-value pairs in a transient state that is
  * accessible only to the next action and is expired immediately after that.
  * This is especially useful when using the POST-REDIRECT-GET pattern to trace
...
trait FlashMapSupport extends Handler {
  ...
  /** Override to implement custom session retriever, or sanity checks if
  ...
  def flash(key: String)(implicit request: HttpServletRequest): Any =
    getFlash(request)(key)

  /** Determines whether unused flash entries should be swept. The default is
    * false.
    */
