...
*/
  ...
  private def isValidRoute(implicit ctx: ActionContext): Boolean = routes.matchingMethods(requestPath).nonEmpty
  private def isPreflightRequest(implicit ctx: ActionContext) = {
  ...
  abstract override def handle(req: HttpServletRequest, res: HttpServletResponse) {
    withRequestResponse(req, res) {
      ...
      request.requestMethod match {
        case Options if isPreflightRequest(ActionContext(req, res)) ⇒ {
