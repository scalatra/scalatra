...
/** A route is a set of matchers and an action. A route is considered to match
  * if all of its route matchers return Some. If a route matches, its action may
  * be invoked. The route parameters extracted by the matchers are made
...
case class Route(
  ...
  /** Optionally returns this route's action and the multi-map of route
    * parameters extracted from the matchers. Each matcher's returned params are
    * merged into those of the previous. If any matcher returns None, None is
    * returned. If there are no route matchers, some empty map is returned.
    */
  ...
  /** The reversible matcher of a route is the first reversible matcher, if any.
    * This matcher may be used to generate URIs.
    */
