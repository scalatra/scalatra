...
/** A route matcher is evaluated in the context it was created and returns a a
  * (possibly empty) multi-map of parameters if the route is deemed to match.
  */
...
final class SinatraRouteMatcher(pattern: String)
  ...
  case class Builder(
    ...
    def addSplat: Builder =
      copy(path = path + splats.head, splats = splats.tail)
...
/** A route matcher for regular expressions. Useful for cases that are more
  * complex than are supported by Sinatra- or Rails-style routes.
  */
final class RegexRouteMatcher(regex: Regex) extends RouteMatcher {
  ...
  /** Evaluates the request path against the regular expression.
    *
    * @return
    *   If the regex matches the request path, returns a list of all captured
    *   groups in a "captures" variable. Otherwise, returns None.
    */
  def apply(requestPath: String): Option[MultiParams] =
...
/** A route matcher on a boolean condition. Does not extract any route
  * parameters.
  */
final class BooleanBlockRouteMatcher(block: => Boolean) extends RouteMatcher {
  ...
  /** Evaluates the block.
    ...
    */
  def apply(requestPath: String): Option[MultiParams] =
    if (block) Some(Map.empty) else None

  override def toString: String = "[Boolean Guard]"

}

final class StatusCodeRouteMatcher(codes: Range, responseStatus: => Int)
    extends RouteMatcher {

  def apply(requestPath: String): Option[MultiParams] =
    if (codes.contains(responseStatus)) Some(Map.empty) else None
