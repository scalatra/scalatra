...
trait ApiFormats extends ScalatraBase {
  ...
  private def parseAcceptHeader(request: HttpServletRequest) = {
   ...
   * one.
   */
  protected def inferFromFormats(implicit req: HttpServletRequest, resp: HttpServletResponse): ContentTypeInferrer = {
    case _ if responseFormat.nonBlank => formats.get(responseFormat) getOrElse "application/octet-stream"
  }

  override protected def contentTypeInferrer(implicit req: HttpServletRequest, resp: HttpServletResponse): ContentTypeInferrer = {
    inferFromFormats orElse super.contentTypeInferrer(req, resp)
  }

  protected def acceptedFormats(accepted: Symbol*)(implicit resp: HttpServletResponse): Boolean = {
  ...
  protected override def withRouteMultiParams[S](req: HttpServletRequest, matchedRoute: Option[MatchedRoute])(thunk: => S): S = {
    val originalParams = multiParams(req)
    val routeParams: Map[String, Traversable[String]] = matchedRoute.map(_.multiParams).getOrElse(Map.empty).map {
      case (key, values) =>
        key -> values.map(s => if (s.nonBlank) UriDecoder.secondStep(s) else s)
    }
    if (routeParams.contains("format")) req(FormatKey) = routeParams.apply("format").head
    req(MultiParamsKey) = originalParams ++ routeParams
    try {
      thunk
    } finally {
