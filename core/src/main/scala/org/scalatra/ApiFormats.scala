...
/** Adds support for mapping and inferring formats to content types.
  *
  * $ - Provides a request-scoped format variable $ - Maps formats to content
  * types and vice versa $ - Augments the content-type inferrer to use the
  * format
  */
trait ApiFormats extends ScalatraBase {
  ...
  private def parseAcceptHeader(implicit
    ...
    def isValidQPair(a: Array[String]) =
      a.length == 2 && a(0) == "q" && Try(a(1).toDouble).toOption
        .exists(validRange.contains)
  ...
  protected[scalatra] override def withRouteMultiParams[S](
    ...
    val routeParams: Map[String, Seq[String]] =
      matchedRoute.map(_.multiParams).getOrElse(Map.empty)
  ...
  /** Returns the request-scoped format. If not explicitly set, the format is: $
    * \- the `format` request parameter, if present in `formatParams` $ - the
    * first match from `Accept` header, looked up in `mimeTypes` $ - the format
    * from the `Content-Type` header, as looked up in `mimeTypes` $ - the
    * default format
    */
  def format(implicit
      request: HttpServletRequest,
