...
class RouteRegistry {
   ...
   */
  def matchingMethods(requestPath: String)(implicit ctx: ActionContext): Set[HttpMethod] = matchingMethodsExcept(requestPath) { _ => false }
   ...
   */
  def matchingMethodsExcept(method: HttpMethod, requestPath: String)(implicit ctx: ActionContext): Set[HttpMethod] = {
  ...
  private def matchingMethodsExcept(requestPath: String)(p: HttpMethod => Boolean)(implicit ctx: ActionContext) = {
