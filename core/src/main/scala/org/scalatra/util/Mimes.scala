...
trait Mimes {
  ...
  /** Detects the MIME type of a given File.
    *
    * When appropriate MIME Type can not be inferred, "application/octet-stream"
    * is returned.
    *
    * This method guesses the MIME type using
    * `java.net.URLConnection.guessContentTypeFromName`. Therefore, by defining
    * an arbitrary MIME type in the configuration file specified by the
    * `content.types.user.table` property, an arbitrary MIME type can be
    * guessed.
    *
    * @param file
