...
object MapQueryString {
  ...
  def apply(rawValue: String): MapQueryString =
    new MapQueryString(parseString(rawValue).toSeq, rawValue)
}
case class MapQueryString(
  ...
  def normalize: MapQueryString = copy(
    SortedMap(
      (initialValues filter (k =>
        !MapQueryString.DEFAULT_EXCLUSIONS.contains(k._1)
      ))*
    ).toSeq
  )
  ...
  private def mkString(values: Value = value) = values map { case (k, v) =>
    v.map(s =>
      "%s=%s".format(
        UrlCodingUtils.queryPartEncode(k),
        UrlCodingUtils.queryPartEncode(s)
      )
    ).mkString("&")
  } mkString "&"
