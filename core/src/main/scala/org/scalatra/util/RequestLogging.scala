...
/** Logs request information using slf4j with "REQUEST" logger name by mixing-in
  * this trait to scalatra servlet or filter.
  */
trait RequestLogging extends ScalatraBase with Handler {
  ...
  abstract override def handle(
    ...
    val realMultiParams = req.getParameterMap.asScala.toMap transform {
      (k, v) =>
        v.toIndexedSeq
    }
    withRequest(req) {
  ...
  private[this] def fillMdc()
      : Unit = { // Do this twice so that we get all the route params if they are available and applicable
    MDC.clear()
  ...
  private[this] def cgiParams = request get CgiParamsKey map (_.asInstanceOf[
    Map[String, String]
  ]) getOrElse Map.empty
