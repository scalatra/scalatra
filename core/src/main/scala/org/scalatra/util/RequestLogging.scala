...
/** Logs request information using slf4j with "REQUEST" logger name by mixing-in this trait to scalatra servlet or
  * filter.
  */
trait RequestLogging extends ScalatraBase with Handler {
  ...
  private[this] def fillMdc()
    ...
    MDC.put(RequestParams, multiParams map { case (k, vl) => vl.map(v => s"${%-(k)}=${%-(v)}") } mkString "&")
    MDC.put(SessionParams, session.dumpAll)
    MDC.put(CgiParams, cgiParams map { case (k, v) => s"${%-(k)}=${%-(v)}" } mkString "&")
  }
