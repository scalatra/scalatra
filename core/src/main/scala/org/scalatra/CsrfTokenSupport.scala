...
/** Provides cross-site request forgery protection.
  ...
  * `handleForgery()` hook is invoked. Otherwise, a token for the next request
  * is prepared with `prepareCsrfToken`.
  */
trait CsrfTokenSupport { this: ScalatraBase =>
  ...
  /** Take an action when a forgery is detected. The default action halts
    * further request processing and returns a 403 HTTP status code.
    */
  protected def handleForgery(): Unit =
    halt(403, "Request tampering detected!")

  /** Prepares a CSRF token. The default implementation uses `GenerateId` and
...
trait XsrfTokenSupport { this: ScalatraBase =>
  ...
  def xsrfGuard(only: RouteTransformer*): Unit =
    before(((only.toSeq ++ Seq[RouteTransformer](isForged)))*) {
      handleForgery()
    }
  ...
  /** Prepares a XSRF token. The default implementation uses `GenerateId` and
    * stores it on the session.
    */
