...
/** FileUploadSupport can be mixed into a [[org.scalatra.ScalatraFilter]] or
  * [[org.scalatra.ScalatraServlet]] to provide easy access to data submitted as
  * part of a multipart HTTP request. Commonly this is used for retrieving
  * uploaded files.
  *
  * Once the trait has been mixed into your handler, you need to enable
  * multipart configuration in your ''web.xml'' or by using `@MultipartConfig`
  * annotation. To configure in ''web.xml'' add `<multipart-config />` to your
  * `<servlet>` element. If you prefer annotations instead, place
  * `@MultipartConfig` to your handler. Both ways provide some further
  * configuration options, such as specifying the max total request size and max
  * size for invidual files in the request. You might want to set these to
  * prevent users from uploading too large files.
  *
  * When the configuration has been done, you can access any files using
  * `fileParams("myFile")` where ''myFile'' is the name of the parameter used to
  * upload the file being retrieved. If you are expecting multiple files with
  * the same name, you can use `fileMultiParams("files[]")` to access them all.
  *
  * To handle any errors that are caused by multipart handling, you need to
  * configure an error handler to your handler class:
  *
  * {{{
  ...
  * }}}
  *
  * }}* @note Once any handler with FileUploadSupport has accessed the request,
  * the fileParams returned by FileUploadSupport will remain fixed for the
  * lifetime of the request.
  *
  * @note
  *   Will not work on Jetty versions prior to 8.1.3. See
...
trait FileUploadSupport extends ServletBase with HasMultipartConfig {
  ...
  protected def isSizeConstraintException(e: Exception): Boolean = e match {
    ...
    case exc: IllegalStateException =>
      exc.getMessage.matches("^Multipart Mime part .+ exceeds max filesize$")
    ...
    case exc: ServletException =>
      ...
      rootCause.getMessage == "400: bad multipart" && (rootCause.getCause match {
        case exc2: java.util.concurrent.CompletionException =>
          exc2.getCause match {
            case exc3: IllegalStateException =>
              exc3.getMessage.matches("^max file size exceeded: \\d+$")
            case _ => false
          }
        case _ => false
      })
  ...
  /** @return
    *   a Map, keyed on the names of multipart file upload parameters, of all
    *   multipart files submitted with the request
    */
...
case class FileItem(part: Part) {
  ...
  val name: String =
    FileItemUtil.partAttribute(part, "content-disposition", "filename")
  val contentType: Option[String] = part.getContentType.blankOption
  val charset: Option[String] =
    FileItemUtil.partAttribute(part, "content-type", "charset").blankOption
