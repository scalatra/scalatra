...
case class RichServletContext(sc: ServletContext) extends AttributesMap {
  ...
  /** Optionally returns a URL to the resource mapped to the given path. This is
    * a wrapper around `getResource`.
    *
    * @param path
    *   the path to the resource
    * @return
    *   the resource located at the path, or None if there is no resource at
    *   that path.
    */
  ...
  /** Optionally returns the resource mapped to the request's path.
    ...
    *   the request
    * @return
    *   the resource located at the result of concatenating the request's
    *   servlet path and its path info, or None if there is no resource at that
    *   path.
    */
  ...
  /** Mounts a handler to the servlet context. Must be an HttpServlet or a
    ...
    *
    * @param urlPattern
    *   the URL pattern to mount. Will be appended with `\/\*` if not already,
    *   as path-mapping is the most natural fit for Scalatra. If you don't want
    *   path mapping, use the native Servlet API.
    *
    * @param name
  ...
  def environment: String =
    sys.props.get(EnvironmentKey) orElse Option(
      sc.getInitParameter(EnvironmentKey)
