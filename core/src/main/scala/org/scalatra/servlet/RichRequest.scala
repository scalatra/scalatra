...
case class RichRequest(r: HttpServletRequest) extends AttributesMap {
  ...
  /** The remainder of the request URL's "path", designating the virtual "location" of the request's target within the
    * application. This may be an empty string, if the request URL targets the application root and does not have a
    * trailing slash.
    */
  ...
  /** A map of headers. Multiple header values are separated by a ',' character. The keys of this map are
    * case-insensitive.
    */
  object headers {
  ...
  /** When combined with scriptName, pathInfo, and serverPort, can be used to complete the URL. Note, however, that the
    * "Host" header, if present, should be used in preference to serverName for reconstructing the request URL.
    */
  def serverName: String = r.getServerName

  /** When combined with scriptName, pathInfo, and serverName, can be used to complete the URL. See serverName for more
    * details.
    */
  ...
  /** Caches and returns the body of the request. The method is idempotent for any given request. The result is cached
    * in memory regardless of size, so be careful. Calling this method consumes the request's input stream.
    *
    * Also note that this method gets data from the input stream of the request, so it may be already consumed according
    * to the servlet specification 3.1.1.
    *
    * @return
  ...
  def multiCookies: MultiParams = {
    Option(r.getCookies)
      .getOrElse(Array.empty[ServletCompat.http.Cookie])
      .toSeq
      .groupBy { _.getName }
  ...
  /** Returns a map of cookie names to values. If multiple values are present for a given cookie, the value is the first
    * cookie of that name.
    */
  ...
  /** The remote address the client is connected from. This takes the load balancing header X-Forwarded-For into account
    ...
    */
  def remoteAddress: String = header("X-FORWARDED-FOR")
    .flatMap(_.split(",").map(_.trim).filterNot(_.isBlank).headOption)
    .getOrElse(r.getRemoteAddr)
