...
/** ServletBase implements the Scalatra DSL with the Servlet API, and can be a
  * base trait of a Servlet or a Filter.
  */
trait ServletBase extends ScalatraBase with SessionSupport with Initializable {
  ...
  protected implicit def configWrapper(config: ConfigT): Config = new Config {
    ...
    def getInitParameterOption(key: String): Option[String] = Option(
