...
object ScalatraBase {
  ...
  /** A key for request attribute that contains any exception that might have
    * occured before the handling has been propagated to ScalatraBase#handle
    * (such as in FileUploadSupport)
    */
  ...
  ): Unit = addCallback(_.failed.foreach(fn))
  ...
  def onRenderedSuccess(fn: Any => Unit)(implicit
      request: HttpServletRequest
  ): Unit = addRenderCallback(_.foreach(fn))

  def onRenderedFailure(fn: Throwable => Unit)(implicit
      request: HttpServletRequest
  ): Unit = addRenderCallback(
    _.failed.foreach(fn)
  )
...
/** The base implementation of the Scalatra DSL. Intended to be portable to all
  * supported backends.
  */
trait ScalatraBase
  ...
  override def handle(
    ...
    request(CookieSupport.SweetCookiesKey) =
      new SweetCookies(request.cookies.toMap, response)
  ...
  /** Executes routes in the context of the current request and response.
    *
    * $ 1. Executes each before filter with `runFilters`. $ 2. Executes the
    * routes in the route registry with `runRoutes` for the request's method.
    *   a. The result of runRoutes becomes the _action result_. b. If no route
    *      matches the requested method, but matches are found for other
    *      methods, then the `doMethodNotAllowed` hook is run with each matching
    *      method. c. If no route matches any method, then the `doNotFound` hook
    *      is run, and its return value becomes the action result. $ 3. If an
    *      exception is thrown during the before filters or the route $ actions,
    *      then it is passed to the `errorHandler` function, and its $ result
    *      becomes the action result. $ 4. Executes the after filters with
    *      `runFilters`. $ 5. The action result is passed to `renderResponse`.
    */
  ...
  protected def renderUncaughtException(
      e: Throwable
  )(implicit
      request: HttpServletRequest,
      response: HttpServletResponse
  ...
  /** Invokes each filters with `invoke`. The results of the filters are
    * discarded.
    */
  ...
  /** Lazily invokes routes with `invoke`. The results of the routes are
    * returned as a LazyList.
    */
  protected def runRoutes(routes: Iterable[Route]): LazyList[Any] =
    for {
      route <- routes.to(
        LazyList
      ) // to(LazyList) makes it lazy so we stop after match
  ...
  /** Invokes a route or filter. The multiParams gathered from the route
    * matchers are merged into the existing route params, and then the action is
    * run.
    *
    * @param matchedRoute
    ...
    *
    * @return
    *   the result of the matched route's action wrapped in `Some`, or `None` if
    *   the action calls `pass`.
    */
  ...
  /** Called if no route matches the current request for any method. The default
    * implementation varies between servlet and filter.
    */
  ...
  /** Called if no route matches the current request method, but routes match
    * for other methods. By default, sends an HTTP status of 405 and an `Allow`
    * header containing a comma-delimited list of the allowed methods.
    */
  ...
  /** The error handler function, called if an exception is thrown during before
    * filters or the routes.
    */
  ...
  /** Renders the action result to the response. $ - If the content type is
    * still null, call the contentTypeInferrer. $ \- Call the render pipeline on
    * the result.
    */
  ...
  /** A partial function to infer the content type from the action result.
    *
    * @return
    *   $ - "text/plain" for String $ - "application/octet-stream" for a byte
    *   array $ - "text/html" for any other result
    */
  ...
  /** The render pipeline is a partial function of Any => Any. It is called
    * recursively until it returns (). () indicates that the response has been
    * rendered.
    */
  ...
  /** Pluggable way to convert a path expression to a route matcher. The default
    * implementation is compatible with Sinatra's route syntax.
    *
    * @param path
    *   a path expression
    * @return
  ...
  /** Path pattern is decoupled from requests. This adapts the PathPattern to a
    * RouteMatcher by supplying the request path.
    */
  ...
  /** Converts a boolean expression to a route matcher.
    ...
    *
    * @return
    *   a route matcher based on `block`. The route matcher should return `Some`
    *   if the block is true and `None` if the block is false.
    *
    * @see
  ...
  /** Prepends a new route for the given HTTP method.
    *
    * Can be overriden so that subtraits can use their own logic. Possible
    * examples: $ - restricting protocols $ - namespace routes based on class
    * name $ - raising errors on overlapping entries.
    *
  ...
  /** Removes _all_ the actions of a given route for a given HTTP method. If
    * addRoute is overridden then this should probably be overriden too.
    *
    * @see
  ...
  /** Initializes the kernel. Used to provide context that is unavailable when
    * the instance is constructed, for example the servlet lifecycle. Should set
    * the `config` variable to the parameter.
    *
    * @param config
    *   the configuration.
    */
  ...
  /** Returns a context-relative, session-aware URL for a path and specified
    * parameters. Finally, the result is run through `response.encodeURL` for a
    * session ID, if necessary.
    *
    * @param path
    *   the base path. If a path begins with '/', then the context path will be
  ...
  /** Builds a full URL from the given relative path. Takes into account the
    * port configuration, https, ...
    *
    * @param path
    *   a relative path
    *
  ...
  /** Gets an init parameter from the config.
    ...
    *
    * @return
    *   an option containing the value of the parameter if defined, or `None` if
  ...
  /** A boolean flag representing whether the kernel is in development mode. The
    * default is true if the `environment` begins with "dev", case-insensitive.
    */
  def isDevelopmentMode: Boolean = environment.toUpperCase.startsWith("DEV")

  /** The effective path against which routes are matched. The definition varies
    * between servlets and filters.
    */
  ...
  def multiParams(key: String)(implicit
      request: HttpServletRequest
  ): Seq[String] = multiParams(request)(key)

  /** The current multiparams. Multiparams are a result of merging the standard
    * request params (query string or post params) with the route parameters
    * extracted from the route matchers of the current route. The default value
    * for an unknown param is the empty sequence. Invalid outside `handle`.
    */
  ...
  /** Set whether to decode when the path returned by the `requestPath` method
    * is Percent-Eoconding. Default is decode (true).
    */
