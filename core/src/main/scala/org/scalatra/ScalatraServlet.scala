...
/** An implementation of the Scalatra DSL in a servlet. This is the recommended
  ...
  * you want Scalatra to have complete control of unmatched requests. $ - you
  * think you want a filter just for serving static content with the default
  * servlet; ScalatraServlet can do this too $ - you don't know the difference
  *
  * @see
  *   ScalatraFilter
  */
trait ScalatraServlet extends HttpServlet with ServletBase with Initializable {
  ...
  /** Defines the request path to be matched by routers. The default definition
    * is optimized for `path mapped` servlets (i.e., servlet mapping ends in
    * `&#47;*`). The route should match everything matched by the `&#47;*`. In
    * the event that the request URI equals the servlet path with no trailing
    * slash (e.g., mapping = `/admin&#47;*`, request URI = '/admin'), a '/' is
    * returned.
    *
  ...
  /** Attempts to find a static resource matching the request path. Override to
    * return None to stop this.
    */
  ...
  protected def resourceNotFound()(implicit
    ...
    if (isDevelopmentMode) {
      ...
      response.getWriter println error.format(
        request.getMethod,
        Option(
          StringEscapeUtils.escapeHtml4(request.getPathInfo)
        ) getOrElse "/",
