...
trait CoreDsl
  ...
  /** Adds a filter to run before the route. The filter only runs if each
    * routeMatcher returns Some. If the routeMatchers list is empty, the filter
    * runs for all routes.
    */
  ...
  /** Defines a block to run if no matching routes are found, or if all matching
    * routes pass.
    */
  ...
  /** Defines an error handler for exceptions thrown in either the before block
    * or a route action.
    *
  ...
  /** The Scalatra DSL core methods take a list of [[org.scalatra.RouteMatcher]]
    * and a block as the action body. The return value of the block is rendered
    * through the pipeline and sent to the client as the response body.
    *
    ...
    * strings and regular expressions to [[org.scalatra.RouteMatcher]], so you
    * can write code naturally.
    * {{{
  ...
  /** Error handler for HTTP response status code range. You can intercept every
    * response code previously specified with #status or even generic 404 error.
    * {{{
    *   trap(403) {
