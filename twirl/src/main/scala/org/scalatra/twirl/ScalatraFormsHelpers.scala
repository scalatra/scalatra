...
trait ScalatraFormsHelpers {
  ...
  /** Render a text field.
    */
  def text(name: String, attributes: (String, String)*)(implicit
      request: HttpServletRequest
  ): Html =
    Html(s"""<input type="text" name="${escape(name)}" value="${escape(
        param(name)
      )}" ${attrs(attributes*)}>""")

  /** Render a password field.
    */
  def password(name: String, attributes: (String, String)*)(implicit
      request: HttpServletRequest
  ): Html =
    Html(s"""<input type="password" name="${escape(name)}" ${attrs(
        attributes*
      )}>""")

  /** Render a textarea.
    */
  def textarea(name: String, attributes: (String, String)*)(implicit
      request: HttpServletRequest
  ): Html =
    Html(s"""<textarea name="${escape(name)}" ${attrs(attributes*)}>${escape(
        param(name)
      )}</textarea>""")

  /** Render a checkbox.
    */
  def checkbox(name: String, value: String, attributes: (String, String)*)(
      implicit request: HttpServletRequest
  ): Html = {
    val checked = if (params(name).contains(value)) "checked" else ""
    Html(s"""<input type="checkbox" name="${escape(name)}" value="${escape(
        value
      )}" $checked ${attrs(attributes*)}>""")
  }

  /** Render a radio button.
    */
  def radio(name: String, value: String, attributes: (String, String)*)(implicit
      request: HttpServletRequest
  ): Html = {
    val checked = if (param(name) == value) "checked" else ""
    Html(s"""<input type="radio" name="${escape(name)}" value="${escape(
        value
      )}" $checked ${attrs(attributes*)}>""")
  }
  ...
  def select(
    ...
    sb.append(s"""<select name="${escape(name)}" ${if (multiple) "multiple"
      else ""}>""")
    values.foreach { case (value, label) =>
      val selected = if (params(name).contains(value)) "selected" else ""
      sb.append(s"""<option value="${escape(value)}" $selected>${escape(
          label
        )}</option>""")
    }
    sb.append("</select>")
  ...
  def errors(name: String)(implicit request: HttpServletRequest): Seq[String] =
    Option(request.getAttribute(RequestAttributeErrorsKey))
      .map { errors =>
        errors.asInstanceOf[Seq[(String, String)]].collect {
          case error if error._1 == name => error._2
        }
      }
      .getOrElse(Nil)
  ...
  private def params(
    ...
    Option(request.getAttribute(RequestAttributeParamsKey))
      .flatMap { params =>
        params.asInstanceOf[MultiParams].get(name)
      }
      .getOrElse(Nil)

  private def param(name: String)(implicit
      request: HttpServletRequest
  ): String =
...
/** Use helper methods defined in `ScalatraFormsHelpers` in Twirl templates as
  * follows:
  *
  * {{{
