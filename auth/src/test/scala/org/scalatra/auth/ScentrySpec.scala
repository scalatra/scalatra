...
//    val theSession = smartMock[HttpSession]
//    implicit val appContext = smartMock[AppContext]
//    val context = new ScalatraApp with SessionSupport {
//      private val sessionMap = scala.collection.mutable.HashMap[String, Any](Scentry.scentryAuthKey -> "6789")
//      override val session = theSession
//      session(anyString) answers { k => sessionMap.get(k.toString).map(_.asInstanceOf[String]).orNull.asInstanceOf[AnyRef] }
//      session.update(anyString(), anyObject()) answers { kv =>
...
//      }
//    }
//    val theScentry = new Scentry[User](context, new SessionAuthStore(theSession), { case User(id) => id }, { case s: String => User(s)})
