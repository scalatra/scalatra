...
object Scentry {
  ...
  private val _globalStrategies =
    new mutable.HashMap[String, StrategyFactory[? <: AnyRef]]()
...
class Scentry[UserType <: AnyRef](
  ...
  private def missingDeserializer: PartialFunction[String, UserType] = {
    case _ =>
      throw new RuntimeException(
        "You need to provide a session deserializer for Scentry"
      )
  }
  ...
  private[this] def runCallbacks(
      guard: StrategyType => Boolean = s => true
  )(which: StrategyType => Unit): Unit =
    strategies foreach {
