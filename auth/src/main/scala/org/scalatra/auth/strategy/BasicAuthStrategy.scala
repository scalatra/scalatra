    ...
    def credentials = {
      if (_credentials.isEmpty)
        _credentials = params map { p =>
          new String(Base64.getDecoder.decode(p), Codec.UTF8.charSet)
            .split(":", 2)
            .foldLeft(null: (String, String)) { (t, l) =>
...
abstract class BasicAuthStrategy[UserType <: AnyRef](
  ...
  implicit def request2BasicAuthRequest(
      r: HttpServletRequest
  ): BasicAuthStrategy.BasicAuthRequest =
