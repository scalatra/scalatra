...
trait JsonImplicitConversions extends TypeConverterSupport {
  ...
  implicit def jsonToSeq[T](implicit
    ...
    implicit val m: Manifest[List[T]] =
      ManifestFactory.classType[List[T]](classOf[List[T]], mf)
  ...
  implicit class jsonToSeqConversion(source: JValue) {
    def asSeq[T](implicit mf: Manifest[T]): Option[Seq[T]] =
