...
class TemplateExample extends ScalatraServlet with FlashMapSupport with ScalateSupport {
  ...
  private def displayPage(title:String, content:Seq[Node])(implicit req: HttpServletRequest, resp: HttpServletResponse) = {
    Template.page(title, content, url(_, includeServletPath = false)(req, resp))
  }
