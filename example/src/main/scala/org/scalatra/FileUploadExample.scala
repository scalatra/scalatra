...
class FileUploadExample extends ScalatraServlet with FileUploadSupport with FlashMapSupport {
  ...
  def displayPage(content: Seq[Node])(implicit req: HttpServletRequest, resp: HttpServletResponse) =
    Template.page("File upload example", content, url(_, includeServletPath = false)(req, resp))
