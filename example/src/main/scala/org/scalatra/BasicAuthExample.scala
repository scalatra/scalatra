...
object BasicAuthExample {
  ...
  trait AuthenticationSupport extends ScentrySupport[MyUser] with BasicAuthSupport[MyUser] {
    ...
    override protected def configureScentry(req: HttpServletRequest, resp: HttpServletResponse) = {
      scentry(req, resp).unauthenticated {
        scentry(req, resp).strategies("Basic").unauthenticated()(req, resp)
      }
    }

    override protected def registerAuthStrategies(req: HttpServletRequest, resp: HttpServletResponse) = {
      scentry(req, resp).register("Basic", new OurBasicAuthStrategy(_, realm))
    }
