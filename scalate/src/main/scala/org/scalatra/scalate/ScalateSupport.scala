...
trait ScalateSupport extends ScalatraKernel {
   ...
   */
  protected def createRenderContext(req: HttpServletRequest, resp: HttpServletResponse , out: PrintWriter): RenderContext = {
...
//    }
  ...
  override protected def renderUncaughtException(req: HttpServletRequest, resp: HttpServletResponse, e: Throwable) {
    if (isScalateErrorPageEnabled) renderScalateErrorPage(req, resp, e)
    else super.renderUncaughtException(req, resp, e)
  }
