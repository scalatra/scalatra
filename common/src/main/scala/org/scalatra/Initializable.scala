...
trait Initializable {
  ...
  /** A hook to initialize the class with some configuration after it has been
    * constructed.
    *
    * Not called init because GenericServlet doesn't override it, and then we
    * get into https://lampsvn.epfl.ch/trac/scala/ticket/2497.
    */
  def initialize(config: ConfigT): Unit

  /** A hook to shutdown the class. Bridges the gap between servlet's destroy
    * and filter's destroy.
    */
