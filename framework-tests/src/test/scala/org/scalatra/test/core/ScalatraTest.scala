...
class ScalatraTestServlet extends ScalatraApp with SessionSupport {
  get("/") {
    "root"
  }
  ...
  get("/print_referrer") {
    (request.headers.get("Referer")) getOrElse ("NONE")
  }
  ...
  get("/init-param/:name") {
    appContext.get(params("name")).toString
  }
}

abstract class ScalatraTest extends ScalatraFunSuite {
  backend.appContext("bofh-excuse") = "decreasing electron flux"
  mount(new ScalatraTestServlet)
  ...
  test("GET /redirect redirects to /redirected") {
    get("/redirect") {
      ...
      headers("Location").split(";").head should endWith ("/redirected")
    }
  }
